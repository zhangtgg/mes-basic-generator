package cn.tfinfo.microservice.mes.basic.generator.common.service.impl;
...
public class BaseServiceImpl<M extends BaseMapper<T>, T> extends ServiceImpl<M, T> implements BaseService<T> {
    ...
    protected QueryWrapper<T> getGenListWrapper(Query query) {
        QueryWrapper<T> wrapper = new QueryWrapper<>();
        wrapper.like(StrUtil.isNotBlank(query.getTableName()), "project_name", query.getTableName());
        wrapper.like(StrUtil.isNotBlank(query.getAttrType()), "module_name", query.getAttrType());
        wrapper.like(StrUtil.isNotBlank(query.getColumnType()), "function_description", query.getColumnType());
